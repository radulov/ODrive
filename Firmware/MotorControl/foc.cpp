...
Motor::Error FieldOrientedController::on_measurement(
    ...
    if (std::isnan(phase_) || std::isnan(phase_vel_)) {
        return Motor::ERROR_UNKNOWN_PHASE;
    }

    // Park transform
    float I_phase = phase_ + phase_vel_ * ((float)(input_timestamp - timestamp_) / (float)TIM_1_8_CLOCK_HZ);
    ...
    // Current error
    float Ierr_d = Id_setpoint_ - Id;
    float Ierr_q = Iq_setpoint_ - Iq;
    ...
    if (enable_current_control_) {
        ...
        // Apply PI control
        Vd = Vd_setpoint_ = v_current_control_integral_d_ + Ierr_d * p_gain_;
        Vq = Vq_setpoint_ = v_current_control_integral_q_ + Ierr_q * p_gain_;
    } else {
        Vd = Vd_setpoint_;
        Vq = Vq_setpoint_;
    }
